...
export function PhoneNumberInput(props: PhoneNumberInputProps) {
	...
	return (
		<div>
			...
			{props.label ? (
				<label htmlFor='phone' className={cn('text-labelColor block text-sm font-medium leading-6', props.labelClassName)}>
					{props.label}&nbsp;
			...
			<div className='flex w-full items-center justify-stretch gap-2'>
				<SingleSelectInput
					...
					render={({ focus, option, selected }) => (
						<div className='flex'>
							<span className={cn('min-w-7 truncate', selected ? 'font-semibold' : 'font-normal')}>{option.value}</span>
							<span className={cn('ml-2 truncate', focus ? 'text-indigo-200' : 'text-gray-500')}>
