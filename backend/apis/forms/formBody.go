...
func updateFormBody(ctx *fiber.Ctx) error {
	...
	if err := utils.ParseBodyAndValidate(ctx, &reqBody); err != nil {
		go utils.LogError(update_form_body_fail, userId, workspaceId, repository.FormObjectType, utils.LogData{"error": err.Error()})
	...
	if len(validationErrors) > 0 {
		...
		go utils.LogError(update_form_body_fail, userId, workspaceId, repository.FormObjectType, utils.LogData{"error": string(jsonStr)})
	...
	if err = queries.UpdateFormBody(ctx.Context(), repository.UpdateFormBodyParams{ID: form.ID, FormBody: reqBody.FormBody}); err != nil {
		go utils.LogError(update_form_body_fail, userId, workspaceId, repository.FormObjectType, utils.LogData{"error": err.Error(), "form_id": form.ID.String()})
		return fiber.NewError(fiber.StatusInternalServerError, err.Error())
	}

	go utils.LogError(update_form_body_success, userId, workspaceId, repository.FormObjectType, utils.LogData{"form_id": form.ID.String()})
