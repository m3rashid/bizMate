...
func authCallback(ctx *fiber.Ctx) error {
	...
	createUser := func() error {
		...
		if err = db.Create(&newUser).Error; err != nil {
			fmt.Println(err)
			return errors.New("error=bad_request")
		}
	...
	jwtToken, err := utils.GenerateJWT(existingUser.ID, existingUser.TenantID, existingUser.Email)
	if err != nil {
...
func checkAuth(ctx *fiber.Ctx) error {
	...
	jwtToken, err := utils.GenerateJWT(userId, tenantId, ctx.Locals("email").(string))
	if err != nil {
